    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const vehicleNumbers =  [...new Set(vehicleData.map(item => item.vehicleNo))];
                const driversNames =  [...new Set(vehicleData.map(item => item.driverName))];
                const driversContacts = [...new Set(vehicleData.map(item=> item.driverNumber))];
            ...
            <form className='receipt-items' onSubmit={handleSubmit}>
                <div className='box1'>
                    <div className='lr-detail'>
                        ...
                        <input type="text" id="lrNumber" name="lrNumber" placeholder="LR Number" value={formData.lrNumber} onChange={handleChange} required  readOnly/>
